...
func main() {
	...
	var disclaimerFile, disclaimerHTML, disclaimerB64 string
	var disclaimerAttachment, multiPartInsert string
	var pretextFile, pretextHTML string
	var htmltoo, forceIntoB64, forceIntoBadHTML string
	var remove, removeAll bool
	var replace, replaceWith string
	var xHeader, alterHeader, alterWith, alterMode string
	var alterSigned bool
	var tmpdir, noQmailBounce string
	var verbose, logStdout, logStderr, logSyslog, debug, version bool
	// Flags
	flag.StringVar(&inputFile, "input", "", "Sets the mailpack file to be the filename supplied, if the filename is a single '-' (hyphen) then the mailpack is sourced via stdin and outputted via stdout.")
	flag.StringVar(&disclaimerFile, "disclaimer", "", "Set the plaintext disclaimer source file.")
	flag.StringVar(&disclaimerHTML, "disclaimer-html", "", "Set the HTML disclaimer source file.")
	flag.StringVar(&disclaimerB64, "disclaimer-b64", "", "Set the BASE64 encoded disclaimer source file (implies --force-into-b64).")
	flag.StringVar(&pretextFile, "pretext", "", "Set the plaintext disclaimer source file.")
	flag.StringVar(&pretextHTML, "pretext-html", "", "Set the HTML disclaimer source file.")
	flag.StringVar(&htmltoo, "htmltoo", "", "Sets alterMIME to insert the plaintext disclaimer into")
	flag.StringVar(&forceIntoB64, "force-into-b64", "", "Sets alterMIME to insert disclaimers into BASE64 encoded text segments")
	flag.StringVar(&forceIntoBadHTML, "force-for-bad-html", "", "Force adding of the HTML disclaimer even when HTML is not correctly formatted the HTML portion of the email body ( if there is no explicitly defined HTML dislcaimer, see --disclaimer-html )")
	flag.StringVar(&disclaimerAttachment, "disclaimer-attachment", "", "disclaimer-attachment=<filename> (use $CID in the disclaimer text to reference)")
	flag.StringVar(&multiPartInsert, "multipart-insert", "", "description")
	flag.BoolVar(&remove, "remove", false, "Remove any attachments which match the filename supplied, if the filename text contains a forward-slash '/', then the matching will occur based on content-type headers rather than by filename.")
	flag.BoolVar(&removeAll, "removeall", false, "Remove all attachments")
	flag.StringVar(&replace, "replace", "", "Replace attachments matching the given filename. Requires to be used with --with.")
	flag.StringVar(&replaceWith, "with", "", "Replace the attachments specified by --replace with the file specified.")
	flag.StringVar(&xHeader, "xheader", "", "Insert a header line as specified into the first set of headers.")
	flag.StringVar(&alterHeader, "alter-header", "", "Alter an existing header in the mailpack.  This function modifies the value of the header, as opposed to the header name.")
	flag.StringVar(&alterWith, "alter-with", "", "")
	flag.StringVar(&alterMode, "alter-mode", "", "(prefix|suffix|replace)")
	flag.BoolVar(&alterSigned, "altersigned", false, "Force alterMIME to modify 'signed' emails")
	flag.StringVar(&tmpdir, "tmpdir", "", "where to store temporary files when processing from STDIN")
	flag.StringVar(&noQmailBounce, "no-qmail-bounce", "", "Don't search into email bodies for attachment headers")
	flag.BoolVar(&verbose, "verbose", false, "Describe details of the process occuring")
	flag.BoolVar(&logStdout, "log-stdout", false, "Send all output messages to stdout")
	flag.BoolVar(&logStderr, "log-stderr", false, "Send all output messages to stderr")
	flag.BoolVar(&logSyslog, "log-syslog", false, "Send all output messages to syslog")
	flag.BoolVar(&debug, "debug", false, "Provide greater verbosity and debugging information")
	flag.BoolVar(&version, "version", false, "display the alterMIME version string")
	flag.Parse()

	glb := NewALTERMIMEAPP_globals()
	// FILE *f
	...
	if replace == "" && replaceWith == "" {
		fmt.Printf("Error: Both --replace= and --with= must be set\n\n")
		flag.Usage()
		return
	}

	if inputFile != "" && (alterMode != "" || replace != "" || disclaimerFile != "" || remove || xHeader != "" || pretextFile != "" || pretextHTML != "") {
		log.Printf("Error: Must specify an action for the input file.\n")
		flag.Usage()
		return
	}

	if inputFile == "-" && (replace || disclaimerFile != "" || xHeader != "") {
		log.Printf("Error: reading/writing from stdin/stdout not implemented for --xheader,--disclaimer, or --replace.\n")
		flag.Usage()
		return
	}
	...
	if inputFile == "-" {
		...
		// 	fmt.Fprintf(&scratch,"%s/%ld%d.amtmp", glb.tmpdir, time(NULL), getpid())
		fmt.Fprintf(&scratch, "%s/%ld%d.amtmp", "/tmp")

		glb.input_file = scratch.String()
		f, err := os.Open(glb.input_file)
		if err != nil {
			log.Printf("ERROR: Cannot open file for writing as a temporary file for alterMIME %v", err)
		}
		defer f.Close()
		_, err = scratch.WriteTo(f)
		if err != nil {
			log.Printf("ERROR: Cannot writing to temporary file %v", err)
		}
	}

	if alterMode != "" && alterWith != "" && alterHeader != "" {
		// 	AM_alter_header( glb.input_file, glb.alter_header, glb.alter_with, glb.alter_mode );
	}

	if replace != "" && replaceWith != "" {
		// AM_attachment_replace( glb.input_file, glb.replace, glb.with);
	}
	...
	if remove != false {
		// AM_nullify_attachment(glb.input_file, glb.remove_filename);
	}
	if xHeader != "" {
	...
	if pretextFile != "" {
		// AM_init();
		// AM_set_pretext(1);
		// AM_set_disclaimer_plain( glb.pretext_file, AM_DISCLAIMER_TYPE_FILENAME );
		if pretextHTML != "" {
			// AM_set_disclaimer_HTML(glb.pretext_html_file, AM_DISCLAIMER_TYPE_FILENAME)
			// AM_set_HTMLtoo(1)
		}
		if glb.pretext_b64_file != "" {
			// AM_set_disclaimer_b64( glb.pretext_b64_file, AM_DISCLAIMER_TYPE_FILENAME );
		}
		// AM_add_disclaimer(glb.input_file)
	}

	if glb.input_is_stdin != 0 {
		var scratch bytes.Buffer

		f, err := os.Open(glb.input_file)
		if err != nil {
			fmt.Printf("%s:%d:ERROR: Cannot open alterMIME temporary file '%s' for reading (%v)", scratch, err)
			return
		}
		defer f.Close()
		...
		_, err = scratch.WriteTo(f)
		if err != nil {
			fmt.Printf("%s:%d:ERROR: Cannot write to '%s' (%s)", glb.input_file, err)
			return
		}
	}
	return
...
var ALTERMIMEAPP_default_remove_prefix string = "removed"
var ALTERMIMEAPP_removeall_filename string = ".*"
...
func NewALTERMIMEAPP_globals() *ALTERMIMEAPP_globals {
